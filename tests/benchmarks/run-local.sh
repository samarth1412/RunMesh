...
python3 tests/benchmarks/local.py active --count 1000 --prefix validation-active > "$result_dir/active-workflows.json"
docker compose exec -T postgres psql -U runmesh -d runmesh -Atc \
  "SELECT json_build_object('active_runs',count(*),'active_tasks',sum(task_count)) FROM (SELECT wr.id,count(tr.id) task_count FROM workflow_runs wr JOIN task_runs tr ON tr.workflow_run_id=wr.id WHERE wr.idempotency_key LIKE 'validation-active-%' AND wr.status='RUNNING' GROUP BY wr.id) rows" \
...
docker compose exec -T postgres psql -U runmesh -d runmesh -c \
  "UPDATE task_runs SET status='CANCELLED' WHERE workflow_run_id IN (SELECT id FROM workflow_runs WHERE idempotency_key LIKE 'validation-active-%'); UPDATE workflow_runs SET status='CANCELLED',completed_at=now() WHERE idempotency_key LIKE 'validation-active-%';" >/dev/null

python3 tests/benchmarks/local.py prepare --tasks 10000 --prefix validation-crash > "$result_dir/worker-crash-setup.json"
dispatch_start=$(date +%s)
docker compose up -d scheduler
while [[ $(docker compose exec -T postgres psql -U runmesh -d runmesh -Atc "SELECT count(*) FROM task_runs tr JOIN workflow_runs wr ON wr.id=tr.workflow_run_id WHERE wr.idempotency_key LIKE 'validation-crash-%' AND tr.status IN ('READY','BLOCKED')") != 0 ]]; do sleep 0.1; done
dispatch_end=$(date +%s)
...
while [[ $(docker compose exec -T postgres psql -U runmesh -d runmesh -Atc "SELECT count(*) FROM task_runs tr JOIN workflow_runs wr ON wr.id=tr.workflow_run_id WHERE wr.idempotency_key LIKE 'validation-crash-%' AND tr.status='RUNNING'") == 0 ]]; do sleep 0.1; done
docker compose kill worker-go
docker compose up -d --scale worker-go=20 worker-go
recovery_start=$(date +%s)
while [[ $(docker compose exec -T postgres psql -U runmesh -d runmesh -Atc "SELECT count(*) FROM workflow_runs WHERE idempotency_key LIKE 'validation-crash-%' AND status NOT IN ('SUCCEEDED','FAILED','CANCELLED')") != 0 ]]; do sleep 1; done
recovery_end=$(date +%s)
docker compose exec -T postgres psql -U runmesh -d runmesh -Atc \
  "SELECT json_build_object('scenario','worker_termination_and_lease_recovery','tasks',count(*),'succeeded',count(*) FILTER (WHERE tr.status='SUCCEEDED'),'other',count(*) FILTER (WHERE tr.status!='SUCCEEDED'),'attempts',sum(tr.attempt_count),'recovered_tasks',count(*) FILTER (WHERE tr.attempt_count > 1),'recovery_seconds',$((recovery_end-recovery_start))) FROM task_runs tr JOIN workflow_runs wr ON wr.id=tr.workflow_run_id WHERE wr.idempotency_key LIKE 'validation-crash-%'" \
  > "$result_dir/worker-crash-result.json"

python3 tests/benchmarks/local.py duplicate --prefix validation-duplicate > "$result_dir/duplicate-submission.json"
docker run --rm \
